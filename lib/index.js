...
var _chartjsCdztPlugin = _interopRequireWildcard(require("./chartjs-cdzt-plugin"));
...
describe('#cdzt', function () {
  var DEFAULT_INIT_OBJ = {
    config: {},
    _cdzt: {
  ...
  it('should mutate chartInstance and attach cdzt objects to chartInstance', function () {
    var CHART_INSTANCE = {
      config: {}
    };

    _chartjsCdztPlugin.default.beforeInit(CHART_INSTANCE);

    expect(CHART_INSTANCE).toEqual(DEFAULT_INIT_OBJ);
  });
  it('should mutate chartInstance and attach zoom properties for line chart types', function () {
    var LINE_INIT_OBJ = {
      '_cdzt': {
        'dragDiv': {
  ...
  it('should attach new event listeners in the afterRender hook', function () {
    ...
    _chartjsCdztPlugin.default.afterRender(CHART_INSTANCE);
  ...
  it('should attach new mousemove and mouseup listeners on mousedown', function () {
    var appendChild = jest.fn();
    var removeEventListener = jest.fn();
    var addEventListener = jest.fn();
    var preventDefault = jest.fn();
    var getBoundingClientRect = jest.fn(function () {
      return {
        right: 10
      };
    });
    var mockMouseDownEvent = {
  ...
  it('should slice the provided chartjs data and labels by the %age zoomed', function () {
    var data = {
      datasets: [{
    ...
    expect((0, _chartjsCdztPlugin.sliceData)(data, start, end)).toEqual(slicedData);
  });
});
